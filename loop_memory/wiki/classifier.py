...
_SECURITY_PATTERNS: tuple[tuple[str, str], ...] = (
    ...
    ("credentials", r"凭证|credential|用户名和密码|账号密码"),
    ("secrets", r"密钥|机密|秘密|secret|敏感信息|敏感数据"),
    ("passwords", r"密码|口令|password|passwd|passphrase"),
    ("tokens", r"令牌|访问令牌|token|bearer"),
    ...
    ("web-vulnerabilities", r"xss|跨站脚本|csrf|跨站请求伪造|ssrf|rce|远程代码执行"),
    ("vulnerabilities", r"漏洞|vulnerability|security flaw|安全缺陷|安全漏洞"),
    ("cve", r"\bcve[- ]?\d{4}[- ]?\d{3,}\b"),
    ("authentication", r"鉴权|认证|身份验证|authentication|authorization|授权|访问控制|access control"),
    ("encryption", r"加密|解密|encryption|decrypt(?:ion)?|tls|https"),
    ("integrity", r"哈希|散列|hash(?:ing)?|签名校验|完整性校验|integrity"),
    ("privacy", r"隐私|个人信息|privacy|personal data|pii|gdpr"),
    ("compliance", r"合规|compliance|监管要求|数据保护"),
...
_UNIVERSAL_PATTERNS: tuple[tuple[str, str], ...] = (
    ("must", r"必须|务必|务须|must|shall"),
    ("never", r"切勿|请勿|绝不要|永远不要|禁止|不要|never|do not|don't"),
    ("always", r"始终|总是|always"),
    ("recommendation", r"推荐|建议|推荐做法|recommended|recommend|should"),
